...
- SSG와 유사; 즉 정적 페이지에만 적용
- 빌드 타임에 페이지가 랜더링된 결과를 저장|후에 접속 요청이 오면 이미 패이지를 제공
- 기능에 따라 정적, 동적으로 나뉨
- 동적(Dynamic)은 어떤 페이지가 요청을 매번 할때마다 변경될경우 혹 데이터가 바뀔경우 + 동적 함수(쿠키, 헤더, 쿼리스트링)
- 정적(Static)은 동적이 아니면 모두 정적으로 됨
